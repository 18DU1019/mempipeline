...
### P3.17 多跳召回触发器落地：分量1基线 30% 击穿触发线（2026-09-23 CTO 裁定）

KG 叙事讨论收敛为可测量触发器（MH 三分量，全部走既有采集面，零产品改动）。当日分量1摸底完成：

**基线数据**（`运行台\脚本\_mh_probe.py`，真实镜像 294 篇，10 组人工配对真实关系对，TOP-5 双命中判定）：MH = **3/10 = 30%**，<40% 直接触发线成立。分级 L1 显式 2/4 → L2 半显式 1/4 → L3 隐式 0/2。**失败形态 7/7 同构**：目标 A 稳进 Top-2、目标 B 掉出 Top-5——强单命中吃掉名额、词面弱的关联篇被挤出，TF-IDF+RRF 结构性盲区（非噪声）。

**CTO 裁定修复序**（触发 ≠ 直接上 graph）：① crossref 邻域扩展（命中条目双向引用邻居补进候选池重排，零新依赖，"轻量 KG 门票"兑现点）；② Top-K 扩容试验（k=5→8）；③ graph 层仅当 ①② 均无效再立项。用同一探针复测裁决，MH ≥60% 关触发器。

**golden 常驻化决策**：10 组用例进 `_test_recall_golden.py` 走**报告轨不设闸**（30% 红线会阻塞门禁，沿用 distractors "先报告后升闸"先例），修复达标后升闸。

**方法论沉淀**：竞品分析判定标准落在用户可感知的结构差异（正本可读性）上，落在部署形态（是否跑进程）等表面特征会误判（Cognee 修正实录）；记忆 stem（无空格连写）与 title（空格标点）形态割裂，子串定位须 stem+全文双通道。

**P3.17-b 复测反转（同日深夜，探针六轮实验）**：三个修复假设逐一证伪——① crossref 邻域扩展：MISS 对 A↔B 之间 wiki-link 零边（语义相关未显式化为链接，镜像 link 稀疏度不支持 link 扩展）；② project_id 聚簇扩展：旧笔记 pid 普遍未回填，无数据可用；③ 语义层：Ollama 开态 MH 仍 30%，W_S=0.2 下语义不足以翻转词面格局。④ title 并入假设：生产 main() 本就拼 title+summary（L654），探针初版 corpus 纯正文口径恰好与生产同结果——30% 即生产真实底数。**根因终定位（--detail 分解）**：B 篇 lex=0.590 并不低，rel=0.118 被 W_S=0.2 压制；与 Top-3 差距 0.08 主要在 imp 层级差（中期 0.6 vs 规则/长期 0.9）与 recency 封顶——**是权重格局问题，非信号缺失**。且 Top-3 常含 B 的内容等价姊妹篇（如 P31 实施篇），信息级命中率高于条目级 30%。**裁定**：a) 权重调整全局风险大（golden 锚点/生产 Top-K 联动），走既有 `--compare` A/B 变体机制评估，禁单点改；b) MH 指标 v2 待修订（条目级 vs 信息级判定）；c) 修复算法零投入（扩展无数据可扩展——避免为零边图写算法的零收益工程）。探针教训：临时脚本的意见反馈 bug（rb 字符串恒真）曾致 HIT 标签污染与 80% 假读数，测量代码自身也要最小验证。

**P3.17-c 权重 A/B 裁定与生产落地（同日，`_mh_probe.py --ab` 八变体 + 门禁相）**：进程内改 rc 模块常量（生产文件零改动）扫 6 权重变体 + 2 容量变体，每变体测条目级 MH / MHv2 信息级 / golden 8 锚点，候选变体再过负例/软泄漏/golden 用对/干扰排序四闸。

矩阵结果（10 对，Top-5）：base 3/10；base+k8 3/10（**容量单独零贡献**——MISS 是深沉排名非近失，全局 rank 16~206）；ws03 3/10（W_S 增益有阈值，0.3 不够）；**ws04（W_S 0.2→0.4）5/10**；imp07（中期 0.6→0.7）3/10（**全程零贡献，剔除杠杆**）；ws04+imp07 5/10（imp07 叠加零增益）；ws04+imp07+k8 **7/10**（k8 在 ws04 排序改善之上 +2，B 从 rank 6-8 进窗）。

门禁相：ws04 全绿——负例拒绝 1.00（出域 rel 恒 0.000，den 塌缩修复在 W_S 抬倍下依然有效，NEG_REL_MAX 绝对线无破防）、软泄漏 1.00、干扰排序 1.00、HubUI 版本链 assert True；且 **golden 7/8→8/8**（修复语料增长稀释的 OneQuant 换皮锚点——2026-09-21 补词模式再现，本次靠权重杠杆而非逐锚补词）、**用对率 0.50→0.88**。

结构性论据：ABSTAIN_REL / 规则与 MOC boost 触发线 / VERSION_REL_BOOST 全部为 W_S 派生（=W_S×0.5），lex 空间语义不变；ws04 只改 rec/imp/rel 相对配比，不动任何门槛语义。

**落地**：recall.py `W_S 0.2→0.4`（docstring + recency 封顶注释 + VERSION_REL_BOOST 注释三处同步），生产态完整 golden SMOKE 全绿（召回 8/8、用对 0.875、负例 3/3、干扰 5/5、软泄漏 clear）。回滚 = 改回 0.2。

**MH 指标 v2 裁定（负结果）**：stem 主题 token 交集≥2 的姊妹篇判定在 10 组×8 变体上**零救回**（MHv2 ≡ 条目级），P3.17-b "Top-3 常含姊妹篇"属个案观察非系统现象。MH 指标维持条目级双命中为主口径，姊妹篇启发式搁置（若重启，改用 title/content 相似度而非 stem 词面）。

**触发器状态更新**：MH 基线 30%→**50%**（ws04 落地后），仍处 40-60% 触发带；k=8 作为 CLI 容量选项即得 7/10，生产默认 k=5 不变。分量 2 / 分量 3 摸底与 10 组用例报告轨化见 P3.17-d（同日完结）。

**工程教训**：① 变体恢复用"元组存单一 dict 复用"被 imp07 变体原地改键污染，门禁相一度实测的是污染态——进程内 A/B 的状态恢复必须每次深拷贝回写；② 矩阵列宽（>5/>8）致人工读数把 7/8 误判为 8/8，表格输出要按分隔符解析而非目测。

**P3.17-d 报告轨落成与分量 2/3 数据面摸底（同日）**：10 组用例正本迁入 `_test_recall_golden.py`（`check_mh`，报告轨不设闸、升闸线 >=60%），首跑 **MH=0.50 (5/10)** 与 A/B ws04 行一致（单源校准闭合）；探针 `_mh_probe.py` 改单源引用防双源漂移。MISS 形态规律化：6 个 MISS 中 5 个是 B@None（A 稳进窗、B 深沉出窗），与 P3.17-b 根因定位吻合；L3 隐式级 0/2 仍最难。

分量 2/3 摸底结论（数据面，只读扫 `recall_access.jsonl` / `recall_judgments.jsonl`）：access log 29 行 / 8 天 / 15 唯一 query（CLI+HTTP 双链路均落盘，稀疏是单人低频的结构性事实）；显式多跳形态仅 1/15。judge 判定 3 条、adopted 0 条（mark 习惯未养成，adopt 提醒挂载已就位待发挥作用）。**裁定：分量 2/3 暂不可算，触发器暂锚分量 1（50%，触发带中）；不为零数据写算法（承 P3.17-b 裁定 c 精神），数据靠日常积累。**行为学副产品：单人场景下多跳需求主要呈隐性形态——用户不写「对比 X 和 Y」式 query，合成基线（分量 1）因此是能力的正确测量面，行为频度（分量 2）低不构成需求不存在的证据。
