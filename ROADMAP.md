...
**触发器状态更新**：MH 基线 30%→**50%**（ws04 落地后），仍处 40-60% 触发带；k=8 作为 CLI 容量选项即得 7/10，生产默认 k=5 不变。分量 2（hop-chain 行为频度）/ 分量 3（dispersion 分散采纳）摸底继续；10 组用例进 `_test_recall_golden.py` 报告轨（50% 不设闸）待办。
